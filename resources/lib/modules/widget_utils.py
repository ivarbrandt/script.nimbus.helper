...
def widget_monitor(list_id):
    ...
    window = None
    try:
        delay = float(xbmc.getInfoLabel("Skin.String(nimbus_category_widget_delay)")) / 1000
    except:
        delay = 0.75
    display_delay = (
        xbmc.getInfoLabel("Skin.HasSetting(nimbus_category_widget_display_delay)") == "True"
    )
    stack_id = list_id + "1"
        ...
        while not monitor.abortRequested() and countdown >= 0 and switch_widget:
            ...
            if display_delay:
                stack_label_control.setLabel(
                    "Loading [COLOR {}][B]{{}}[/B][/COLOR] in [B]%0.2f[/B] seconds".format(label_color).format(
                        widget_label
                    )
                    % (countdown)
                )
